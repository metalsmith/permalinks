...
const normalize = options => {
  ...
  options.rename = options.rename === false ? false : true;
  return options;
};
 ...
 * @return {Object}
 */
const family = (file, files, options) => {
  const ret = {};
 ...
 * @return {Object}
 */
const folder = (file, files, options) => {
  ...
  for (const otherFile in files) {
    ...
    if (html(otherFile, options)) continue;
 ...
 * @return {Boolean}
 */
const html = (path, options) => !options.rename || '.html' === extname(path);
...
const plugin = options => {
  ...
  return (files, metalsmith, done) => {
    ...
    const defaultUniquePath = (targetPath, filesObj, filename, opts) => {
      ...
      let counter = 0;
      let postfix = '';
      let indexFileExt = '.html';
      do {
        if (!options.rename) {
          indexFileExt = extname(filename);
          debug('skip renaming extension for: %s', filename);
        }
        target = join(
          `${targetPath}${postfix}`,
          indexFile || 'index' + indexFileExt
        );
    ...
    Object.keys(files).forEach(file => {
      ...
      switch (linkset.relative) {
        case true:
          fam = family(file, files, options);
          break;
        case 'folder':
          fam = folder(file, files, options);
          break;
      }
