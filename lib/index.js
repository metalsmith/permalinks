...
var trSlug = require('transliteration').slugify;
...
function normalize(options){
  ...
  options.transliteration = options.hasOwnProperty('transliteration') ? options.transliteration : false;
  ...
  for (var i = 0, key; key = keys[i++];) {
    ...
    if (val instanceof Date) {
      ...
      if (options.transliteration) {
        ret[key] = trSlug(val.toString());
        debug('tr slug %s', ret[key])
      } else {
