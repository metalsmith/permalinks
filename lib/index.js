...
const folder = (file, files) => {
  const bn = sysPath.basename(file, sysPath.extname(file));
  const family = {};
  let dir = sysPath.dirname(file);
  ...
  const sharedPath = sysPath.join(dir, bn, '/');
 ...
 */
const resolve = str => {
  const base = sysPath.basename(str, sysPath.extname(str));
  let ret = sysPath.dirname(str);

  if (base !== 'index') {
    ret = sysPath.join(ret, base).replace(/\\/g, '/');
  }
 ...
 * @return {Boolean}
 */
const html = str => sysPath.extname(str) === '.html';
      ...
      if (fam) {
        for (const key in fam) {
          if (Object.prototype.hasOwnProperty.call(fam, key)) {
            const rel = sysPath.posix.join(path, key);
            dupes[rel] = fam[key];
            moved[key] = rel;
          }
        }
