  ...
  return function(files, metalsmith, done){
    ...
    Object.keys(files).forEach(function(file){
      ...
      switch (linkset.relative) {
        case true:
          fam = family(file, files, options.move);
          break;
        case 'folder':
          fam = folder(file, files, options.move);
          break;
      }
...
function normalize(options){
  ...
  options.move = options.move || false
...
function family(file, files, doMove){
 ...
 * @return {Object}
 */
function folder(file, files, doMove) {
  var dir = dirname(file);
  ...
  var siblingName = join(dir, bn);

  for (var key in files) {
    if (key === file) continue;
    if (key.indexOf(siblingName) !== 0) continue;
    if (html(key)) continue;
    var rel = key.slice(siblingName.length);
    ret[rel] = files[key];
    if(doMove) delete files[key];
  }
