...
function normalize(options){
  ...
  options.umlauts = options.hasOwnProperty('umlauts') ? options.umlauts : true;
  return options;
}
...
function replace(pattern, data, options){
  ...
  for (var i = 0, key; key = keys[i++];) {
    ...
    if (val instanceof Date) {
      ...
      ret[key] = slug(options.umlauts ? replaceUmlauts(val.toString()) : val.toString());
    }
  }
...
/**
 * replace german umlauts for slugs
 * @param  {String} str
 * @return {String}
 */
function replaceUmlauts(str) {
  var umlautReplacements = {
    'ä': 'ae',
    'ö': 'oe',
    'ü': 'ue',
    'ß': 'ss'
  };
  Object.keys(umlautReplacements).forEach(function(umlaut) {
    var replacement = umlautReplacements[umlaut];
    str = str.replace(umlaut, replacement);
  });
  return str;
}
