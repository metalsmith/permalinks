...
function plugin(options){
  ...
  return function(files, metalsmith, done){
    ...
    Object.keys(files).forEach(function(file){
      ...
      // Save the original filename if `filename`
      // is set on the file
      if (data.filename) {
        originalFilesMapping[out] = data.filename;
      }
    });
    ...
    // store a map of original filenames to their 
    // permalink counterparts
    var metadata = metalsmith.metadata();
    metadata.permalinkMapping = originalFilesMapping;
  };
}
