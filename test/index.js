...
describe('metalsmith-permalinks', function(){
  ...
  it('should format a date when date saved as a string', function(done){
    Metalsmith('test/fixtures/date-by-key')
    .use(permalinks({
      pattern: ':date'
    }))
    .build(function(err){
      if (err) return done(err);
      equal('test/fixtures/date-by-key/expected', 'test/fixtures/date-by-key/build');
      done();
    });
  });
});
